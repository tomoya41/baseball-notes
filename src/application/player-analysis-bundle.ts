...
export class PlayerAnalysisBundleService {
  ...
  async find(playerId: string, asOfDate: string) {
    ...
    const [comparison, homeAway, opponent, battingOrder] = await Promise.allSettled([
    ...
    const payload = playerAnalysisBundleSchema.parse({ playerId, asOfDate,
      ...
      battingOrder: settled(battingOrder) });
    return { payload, context, dbReadMs, aggregationMs };
  }
}
