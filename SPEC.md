...
Initial proof uses Retrosheet's permitted 2025 MLB gameinfo release, with its required prominent attribution, to construct historical division standings; it does not supply 2026 daily data. SQLite-compatible migrations and repository ports allow a local database and a future no-overage Turso free project; neither remote account nor public deployment is created by this task. Local gzip archives and generated static JSON prove the data path. Remote archive durability, current-season source permission, authenticated publishing and prominent UI attribution remain prerequisites before turning the scheduled production collector on. See `docs/data-architecture.md`.
...
The local Home standings slice reads generated and validated own JSON, not nf3 at page-open time. Include source attribution and effective date. Do not treat the locally generated asset as a deployed or Android-updating service. Date-stamped raw captures may be replayed for corrections; never label today's undated standings page as a past day's snapshot. Keep GitHub daily scheduling opt-in until durable free storage, backup, public payload delivery and current source-condition review are in place. See `docs/npb-ingestion.md` for the verified fields and limits.
