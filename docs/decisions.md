...
## 2026-09-25 — Month and observed-season Player periods

Add current month and season to the same read-only Player period service and sum-then-rate engine. Month begins on the first JST calendar day. For season, select the latest season recorded by a final NPB Game and bound reads by that season ID and its earliest recorded final Game, rather than assuming January 1 or a fixed opening date. Because the current Game schema does not prove the actual regular-season opener, keep Season Coverage unknown even if the observed interval is complete; known partial failures remain partial. A future verified opening-date/season-type reader can upgrade that proof without changing Fact formulas or collecting new data in this task. The Player selector uses compact two-row buttons; no migration, collector, workflow, backup, derived cache or HOT change is made.

## 2026-09-25 — Reviewed NPB season calendar and coverage calendar

Supersede the observed-season boundary above with a version-controlled, manually reviewed first-team regular-season record. The 2026 opener is March 27 per NPB's official schedule; October 7 is only the latest *scheduled* regular-season fixture as checked September 25 and must be reviewed after postponements. Static config rather than a migration fits a few annual records, keeps URL/method/verification provenance in Git, and avoids changing the Fact schema or daily collector. A season query now starts at the reviewed opener and caps at the recorded endpoint, while a separate Repository query reports first/last stored Fact dates. The existing Day/Game proof is exposed as a per-date coverage calendar and summary for all five periods; missing days stay unknown, explicit no-games days count as covered, and known failures remain partial. Backfill with verified Day and Game gates can upgrade coverage without changing rates or inserting Derived rows. Rollback reverts the config, resolver, coverage response and UI copy together and redeploys API and client, without touching permanent Facts. See `docs/npb-season-coverage.md`.

## 2026-09-25 — Transparent read-only NPB HOT candidates

Use only the existing all-player seven-day period batch to form HOT candidates. Production requires complete Day/Game coverage, complete ranking inputs, and centralized PA/outs/appearance minimums; excluded players retain explicit diagnostic reasons. Rank batters by OPS and pitchers separately as starters or relievers by ERA, with deterministic sample and canonical-ID tie-breaks. Do not infer WHIP from combined BB+HBP, introduce a composite score, or publish Home HOT while no complete-coverage cohort exists. A manual-only read-only diagnostic verifies Turso without changing collector schedules, Fact schema, cache, or public payloads. Rollback removes the pure candidate evaluator and diagnostic step; stored Facts and Player Recent stay intact. See `docs/npb-hot-candidates.md`.

## 2026-09-26 — Stage versioned HOT JSON without publishing it

Project the existing seven-day HOT Candidate Engine into a strict version-1 static contract. Keep generated time separate from the JST effective date, preserve category readiness for future staged rollout, and leave all public ranking arrays empty unless full Batch Coverage, eligible candidates in all three categories, and actual scheduled-production evidence are present. Mixed category readiness remains diagnostic-only to avoid silently changing Home scope. Validate exact ranks/order and required fields, then stage the JSON by validated temporary-file rename under Git-ignored `.data/`. A manual read-only Action may test Turso, but Daily Workflow, Pages and Home remain unchanged. Rollback removes this projection/writer/manual step without touching Facts or ingestion. See `docs/npb-hot-payload.md`.

## 2026-09-26 — Publish HOT separately and keep the full-site Pages artifact intact

Use a manual-only HOT Pages workflow that reads stored Facts, applies the existing Production Gate, and publishes a validated seven-day JSON plus the current standings and app. The Home HOT section reads this static JSON, never Turso or sample rankings. Because Pages deployments replace the entire site, the Daily workflow's delivery step preserves only a previously published, schema-valid HOT JSON; its ingestion, schedule and backup logic are untouched. The HOT workflow shares the Pages concurrency group, so whole-site deployments cannot interleave. Keep all ranking arrays empty while the gate is closed, including when one category alone is ready. Require reviewed Player name/team metadata before opening the gate, and block stale ready payloads in Home. A future trigger from successful Daily ingestion can be added separately. Rollback removes the independent workflow, preservation step and Home adapter, leaving standings and Fact collection unchanged. See `docs/npb-hot-payload.md`.
...
## 2026-09-27 — Protect full NPB Facts from limited lookback writes

Full, validated Game collection keeps authoritative replace semantics, including real corrections and explicit nulls. Limited player-log collection inserts missing Facts but does not update any existing Game Fact. Its incomplete field set cannot safely correct a full record or erase known PA, sacrifice counts, extra-base hits, batting order or role. This conservative policy also covers pitching and leaves corrections to the full-detail path; zero remains an observed value. No migration is needed. Targeted repair uses canonical Game/Player pairs, full-detail Source reconciliation, known-field equality checks, and a single transaction without creating identities or changing unrelated Facts. Rollback must not re-enable the destructive limited upsert; disable limited Fact persistence if reverting.

The shared 30-day Analysis Context additionally projects batter starter/substitute splits using only the stored nullable starter flag. Missing flags remain unknown, zero-PA appearances remain games, and all rates reuse the existing aggregator. Per-section failure isolation and shared period coverage remain unchanged; no additional Fact read, derived table, HOT change or appearance-order inference is introduced.
