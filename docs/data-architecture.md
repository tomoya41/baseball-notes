...
2026-09-25に既存SchemaのPortable DDL + Table別JSONL.gz Manifest Exportを追加し、Tursoから新規SQLiteへのRestoreとRepository読戻しを演習済み。[手順・対象Table・検証結果](npb-backup-day-proof.md)。Manifestに件数・schema/migration version・SHA-256を入れ、未知の新Tableが増えた場合はExportを失敗させて保全漏れを防ぐ。第6弾でmanual実投入後と統合日次manual run後にもExport→Restoreを確認し、公開repoの短期ArtifactへはAES-256-GCM暗号化済みBackupのみ7日間保存した。これは持続的な非公開オフプロバイダー保管ではなく、鍵の私有保管・長期保全は運用上の課題。
