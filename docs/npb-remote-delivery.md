...
第6弾の前日全試合Factは[日次運用手順](npb-day-operations.md)に分離した。従来のstandings JSON配信は変えず、Player Game FactをPagesへ公開しない。公開リポジトリのActions Artifactは読取可能なので、短期Artifactには暗号化済みExportのみを配置する。manual dry-run、Turso実投入・再投入、Backup、Pages公開のGateを通過し、2026-09-25に既存定刻Workflowの`NPB_DAY_FACTS_ENABLED`を有効化した。統合後のmanual runは成功済み。次回scheduled runでの最終運用確認は残る。
